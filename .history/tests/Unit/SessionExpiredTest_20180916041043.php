...
class ExampleTest extends TestCase
{
    ...
    public function test5RedirectToLoginPageWhenSessionExpired()
    {
        ...
        // $m = Mockery::mock(Auth::class);
        // $m->shouldReceive('user')->withNoArgs()->andReturn(null);
        // $this->app->instance(Auth::class, $m);

        //config(['session.lifetime' => 0]);

        Auth::shouldReceive('user')->withNoArgs()->andReturn(null);
        Config::shouldReceive('offsetGet')->with('session.lifetime')->andReturn(0);
        //dd(Session::all());
        //dd(auth()->user());
