...
class ExampleTest extends TestCase
{
    ...
    {
        config(['session.lifetime' => 1]); // minutes
        ...
        //$m = Mockery::mock(StartSession::class)->shouldAllowMockingProtectedMethods();
        //$m->shouldReceive('getSessionLifetimeInSeconds')->withNoArgs()->andReturn(-80);
        $m = Mockery::mock(DatabaseSessionHandler::class);//->shouldAllowMockingProtectedMethods();
        $m->shouldReceive('expired')->withNoArgs()->andReturn(true);
        $this->app->instance(DatabaseSessionHandler::class, $m);

        //config(['session.lifetime' => 0]);

        //Auth::shouldReceive('user')->withNoArgs()->andReturn(null);
        //Config::shouldReceive('get')->with('session.lifetime')->andReturn(0);
        //dd(Session::all());
        //dd(auth()->user());
